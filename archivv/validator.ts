...
/**
 * La validación de verdad NUNCA vive adentro de Antigravity. Acá se
 * comprueba, desde afuera:
 *   1) si escribió el archivo de resultado que le pedimos (ayuda, no fuente de verdad)
 *   2) qué cambió realmente en el disco según git
 *   3) si los archivos que esperábamos (research.md, etc.) están donde deberían
 */
export async function validateWorkspace(
  ...
  try {
    ...
    // Sin resultado, o JSON inválido. No es fatal por sí solo — seguimos
    // validando por git diff / archivos esperados.
  }
  ...
  try {
    ...
    // No es repo git en este workspace de prueba, o no hay diff todavía.
  }

  let expectedFilesOk = opts.expectedFiles.length === 0 ? true : true;
